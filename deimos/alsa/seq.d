...
struct snd_seq_queue_info_t;
/** queue status container */
struct snd_seq_queue_status_t;
/** queue tempo container */
struct snd_seq_queue_tempo_t;
/** queue timer information container */
struct snd_seq_queue_timer_t;
...
ptrdiff_t snd_seq_event_length(snd_seq_event_t *ev);
...
struct snd_seq_remove_events_t;
 ...
 */
extern const uint snd_seq_event_types[];

auto _SND_SEQ_TYPE(T)(T x){ return (1<<(x)); }    /**< master type - 24bit */
auto _SND_SEQ_TYPE_OPT(T)(T x){ return ((x)<<24); }   /**< optional type - 8bit */

/** check the event type */
auto snd_seq_type_check(T, U)(auto ref T ev, auto ref U x){ return snd_seq_event_types[(ev).type] & _SND_SEQ_TYPE(x); }

/** event type check: result events */
auto snd_seq_ev_is_result_type(T)(auto ref T ev){
    return snd_seq_type_check(ev, SND_SEQ_EVFLG_RESULT);}
/** event type check: note events */
auto snd_seq_ev_is_note_type(T)(auto ref T ev){
    return snd_seq_type_check(ev, SND_SEQ_EVFLG_NOTE);}
/** event type check: control events */
auto snd_seq_ev_is_control_type(T)(auto ref T ev){
    return snd_seq_type_check(ev, SND_SEQ_EVFLG_CONTROL);}
/** event type check: channel specific events */
auto snd_seq_ev_is_channel_type(T)(auto ref T ev){
